...
async function getAnthropicKey() {
  const keyPath = path.join(os.homedir(), '.config', 'anthropic.token');
  try {
    return (await fs.readFile(keyPath, 'utf8')).trim();
  } catch (err) {
    console.error(`Error: Anthropic API key not found at ${keyPath}`);
    console.log('To set up your API key:');
    console.log('1. Sign up for an account at https://www.anthropic.com.');
    console.log('2. Copy your API key from the Anthropic dashboard.');  
    console.log(`3. Save the key to ${keyPath}`);
    process.exit(1);
  }
}
